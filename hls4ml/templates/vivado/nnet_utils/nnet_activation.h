...
#pragma HLS UNROLL
    ...
    if (datareg > 0)
      res[ires * rufactor + ir] = datareg;
    else
      res[ires * rufactor + ir] = 0;
  }
  //}
}
...
LeakyReLUActLoop:
  ...
  DataPrepare:
    for (int c = 0; c < CONFIG_T::n_in; c++) {
      #pragma HLS UNROLL
      data_cache[c] = data[c].read();
    }

    leaky_relu<data_T, res_T, CONFIG_T>(data_cache, alpha, res_cache);

  ResWrite:
    for (int c = 0; c < CONFIG_T::n_in; c++) {
      #pragma HLS UNROLL
      res[c].write(res_cache[c]);
    }
  }
}
