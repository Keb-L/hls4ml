...
void myproject_in(
    ...
    hls::stream<result_t> layer17_out[N_FILT_17], // TODO: layer52_out[N_LAYER_52]
...
//#pragma HLS INTERFACE axis port = em_barrel, layer7_out

    // const_size_in_1 = N_INPUT_1_1 * N_INPUT_2_1 * N_INPUT_3_1;
    // const_size_out_1 = N_FILT_26; //N_LAYER_52; // TODO: N_LAYER_52
    ...
    #pragma HLS STREAM variable=layer8_out depth=100 dim=1
    ...
    // #pragma HLS STREAM variable=layer17_out depth=2 dim=1
    ...
    // #pragma HLS STREAM variable=layer26_out depth=2 dim=1
...
/*
    ...
    #pragma HLS STREAM variable=layer35_out depth=2 dim=1
    ...
    while(!layer46_out[0].empty()) nnet::leaky_relu_stream<layer46_t, layer47_t, LeakyReLU_config47>(layer46_out, 0.30000001192092896, layer47_out);
    ...
    while(!layer50_out[0].empty()) nnet::leaky_relu_stream<layer50_t, layer51_t, LeakyReLU_config51>(layer50_out, 0.30000001192092896, layer51_out);
...
void myproject(
    hls::stream<input_t> em_barrel[N_INPUT_3_1],
    hls::stream<result_t> layer54_out[N_FILT_17], //layer54_out[N_LAYER_52],
    model_weightdefault_t w31[147456],
    model_weightdefault_t w36[294912],
    model_weightdefault_t w40[589824],
    model_weightdefault_t w44[589824])
...
#pragma HLS DATAFLOW
    hls::stream<result_t> layer54b_out[N_FILT_17];
#pragma HLS STREAM variable = layer54b_out depth = 5000 dim = 1

    for (int i0 = 0; i0 < 56 * 11; i0++)
    {
        myproject_in(em_barrel, layer54b_out, w31, w36, w40, w44);
        while (!layer54b_out[0].empty()) {
            for (int i1 = 0; i1 < N_FILT_17; i1++)
            {
                #pragma HLS UNROLL
