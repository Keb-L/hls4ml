...
*/
    bool lReset = true;
    for(unsigned iC = 0; iC < OUT_HEIGHT_102; iC++) { 
     //Read in the input image to bottom row of buffer
     LoopSubstream:
     for(unsigned i1 = 0; i1 < OUT_WIDTH_102; i1++) {
      LoopInput:
      for(unsigned i2 = 0; i2 < N_CHANNEL_102; i2++) { 
       #pragma HLS UNROLL
       if(i1*iC < OUT_HEIGHT_102*OUT_WIDTH_102) {
        sInput[i2].write(em_barrel[iC*OUT_WIDTH_102*N_CHANNEL_102+i1*N_CHANNEL_102+i2]);
	//sInput[i2].write(layer103_out_uf[iC][i1][i2]);
       } else { 
        input_t pVal = 0; 
