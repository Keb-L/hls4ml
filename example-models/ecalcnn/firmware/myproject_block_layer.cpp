...
#include "myproject_block_layer.h"
...
//unsigned short &const_size_out_1
    ...
    const unsigned Nstop = OUT_WIDTH_6*OUT_HEIGHT_6;
        ...
        for(unsigned i1 = 0; i1 < N_INPUT_2_1; i1++) {
            ...
            if(!sOutput[0].empty()) { 
                ...
                for(unsigned iX = 0; iX < N_FILT_6; iX++) { 
                    ...
                    if(index < Nstop) preds[index*N_FILT_6+iX] = (result_t)sOutput[iX].read(); //layer41_out[index*N_FILT_2+iX] = (result_t)sOutput[iX].read();
                }
                index++; 
                // if(index > 9) break;
...
hls::stream<result_t> output[N_FILT_6]   
    ...
    nnet::conv_2d_large_cl_sr<input_t,layer2_t,config2>(iReset,input,layer2_out,w2,b2,s4,b4,alpha);

    std::cout << "IsEmpty? " << layer2_out[0].empty() << std::endl;

    static hls::stream<layer6_t> layer6_out[N_FILT_6];
    #pragma HLS stream variable=layer6_out      depth=1
    if(!layer2_out[0].empty()) nnet::pooling2d_cl<layer2_t, layer6_t, config6>(iReset,layer2_out,output);
